...
import type { ThreadMessage, ThreadPreview } from "@/lib/gmail";
...
export function ThreadReader({
  ...
  const sender = preview ? senderName(preview.from) : "";
  const dateLabel = preview?.dateMs
    ? shortDate(new Date(preview.dateMs).toISOString())
    : thread.last_message_at
    ? shortDate(thread.last_message_at)
    : "";

  // When opened, show only the most recent message; "Show full thread" reveals
  // the whole chain. Gmail returns messages oldest-first, so the latest is last.
  const sorted = [...(messages ?? [])].sort(
    (a, b) => (a.dateMs ?? 0) - (b.dateMs ?? 0)
  );
  const shownMessages = expanded ? sorted : sorted.slice(-1);
    ...
    >
      <div className="flex items-start justify-between gap-2.5 px-3.5 py-3">
        <button onClick={toggle} className="flex min-w-0 flex-1 items-start gap-3 text-left">
          ...
          <span className="min-w-0 flex-1">
            <span className="flex items-baseline justify-between gap-2">
              <span
                className={`truncate text-sm text-text ${unread > 0 ? "font-bold" : "font-semibold"}`}
              >
                {sender || "Email"}
              </span>
              {dateLabel && (
            ...
            <span className="mt-0.5 block truncate text-xs text-text-faint">
              {preview?.snippet || "Open to read this conversation."}
            </span>
          </span>
        </button>
        <div className="flex shrink-0 items-center gap-1.5">
          {unread > 0 && (
            <span
              className="inline-flex min-w-[20px] items-center justify-center rounded-pill bg-accent px-1.5 py-0.5 text-[11px] font-bold leading-none text-accent-fg"
          ...
          <button
            ...
            className="rounded-[8px] p-1 text-text-faint transition hover:bg-red-bg hover:text-red"
