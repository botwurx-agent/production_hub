  ...
  return (
    <div className="w-full rounded-[12px] bg-surface p-4 text-text sm:p-6">
      ...
      {media.length === 0 ? (
        ...
        // One big frame per row so details are actually visible and pinnable.
          ...
          {media.map((m) => (
            ...
            >
              <div className="flex items-center gap-2 border-b border-border px-3 py-2">
                <span
                  className="rounded-[6px] px-2 py-0.5 text-[11px] font-extrabold uppercase tracking-wide"
                  style={{
                    backgroundColor:
                      m.role === "start"
                        ? "var(--h-cyan-bg)"
                ...
                {m.model && (
                  <span className="rounded-[5px] bg-surface-2 px-1.5 py-0.5 text-[10px] font-bold text-text-muted">
                    {m.model}
                  </span>
                )}
                {m.openUrl && (
