...
- AUTH EMAIL BRANDING (docs/launch/supabase-auth-emails.md): signup
  confirmation, password reset and email change are generated by SUPABASE AUTH,
  not by lib/email.ts, so they send from Supabase's shared SMTP with Supabase
  branding until Custom SMTP is pointed at the same Resend account the app
  already uses. The doc holds the SMTP settings plus drop-in templates matching
  lib/email-template.ts, whose links target our own /auth/confirm route with
  token_hash + type so the recipient sees our domain rather than the project's
  supabase.co host. Two of those emails were already reachable in production
  (/forgot-password, resend-confirmation), so this was live regardless of the
  Confirm-email toggle. /auth/confirm also gained a guard on its `next`
  parameter: it went straight from the query string into new URL(), where a
  protocol-relative //host resolves to another origin, so a valid token could
  have been spent on an off-site redirect.
